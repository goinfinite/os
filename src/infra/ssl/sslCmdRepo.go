...
func (repo SslCmdRepo) deleteCurrentSsl(vhost valueObject.Fqdn) error {
	...
	vhostCertFilePath := infraShared.EnvDataInfra.PkiConfDir + "/" + vhostStr + ".crt"
	...
	vhostCertKeyFilePath := infraShared.EnvDataInfra.PkiConfDir + "/" + vhostStr + ".key"
...
func (repo SslCmdRepo) ReplaceWithSelfSigned(vhost valueObject.Fqdn) error {
	...
	return infraHelper.CreateSelfSignedSsl(infraShared.EnvDataInfra.PkiConfDir, vhost.String())
}
...
func (repo SslCmdRepo) isDomainMappedToServer(
	...
	ownershipValidateUrl := "https://" + serverIpAddress.String() +
		infraShared.EnvDataInfra.DomainOwnershipValidationUrlPath
...
func (repo SslCmdRepo) ReplaceWithValidSsl(sslPair entity.SslPair) error {
	path, _ := valueObject.NewMappingPath(infraShared.EnvDataInfra.DomainOwnershipValidationUrlPath)
	...
	vhostCrtFilePath := infraShared.EnvDataInfra.PkiConfDir + "/" + firstVhostStr + ".crt"
	...
	vhostKeyFilePath := infraShared.EnvDataInfra.PkiConfDir + "/" + firstVhostStr + ".key"
	err = infraHelper.CreateSymlink(
		certbotKeyFilePath,
		vhostKeyFilePath,
		shouldOverwrite,
...
func (repo SslCmdRepo) Create(createSslPair dto.CreateSslPair) error {
	...
	firstVhostCertFilePath := infraShared.EnvDataInfra.PkiConfDir + "/" + firstVhostStr + ".crt"
	firstVhostCertKeyFilePath := infraShared.EnvDataInfra.PkiConfDir + "/" + firstVhostStr + ".key"
