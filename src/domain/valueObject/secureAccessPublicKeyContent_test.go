package valueObject

import "testing"

func TestSecureAccessPublicKeyContent(t *testing.T) {
	t.Run("ValidSecureAccessPublicKeyContent", func(t *testing.T) {
		rawValidSecureAccessPublicKeyContent := []any{
			"ssh-rsa  myMachine@pop-os",
			"ssh-rsa ",
			"ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILWHpG24haBESaLR6bwdcnNWy6YPw355jXVYn+DlL6Lc",
			"ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILWHpG24haBESaLR6bwdcnNWy6YPw355jXVYn+DlL6Lc myMachine@pop-os",
		}

		for _, rawKeyContent := range rawValidSecureAccessPublicKeyContent {
			_, err := NewSecureAccessPublicKeyContent(rawKeyContent)
			if err != nil {
				t.Errorf(
					"UnexpectedError: %v, value: '%v'", err.Error(), rawKeyContent,
				)
			}
		}
	})

	t.Run("InvalidSecureAccessPublicKeyContent", func(t *testing.T) {
		rawInvalidSecureAccessPublicKeyContent := []any{
			12345, 1.25, true, "", "ssh-rsa", "ssh-rsa myMachine@pop-os",
			"c2VjdXJlIGFjY2Vzcy/BrZXkgY29udGV/udCB0ZXN0+U= myMachine@pop-os",
		}

		for _, rawKeyContent := range rawInvalidSecureAccessPublicKeyContent {
			_, err := NewSecureAccessPublicKeyContent(rawKeyContent)
			if err == nil {
				t.Errorf("MissingExpectedError: '%v' was accepted", rawKeyContent)
			}
		}
	})

	t.Run("ReadOnlyKeyName", func(t *testing.T) {
		rawValidSecureAccessPublicKeyContentWithName := "ssh-rsa  myMachine@pop-os"
		keyContentWithName, err := NewSecureAccessPublicKeyContent(rawValidSecureAccessPublicKeyContentWithName)
		if err != nil {
			t.Fatalf("SecureAccessPublicKeyContentParseFailed: %v", err)
		}

		_, err = keyContentWithName.ReadOnlyKeyName()
		if err != nil {
			t.Errorf("ReadOnlyKeyNameFailed: %v", err)
		}

		rawValidSecureAccessPublicKeyContentWithoutName := "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAILWHpG24haBESaLR6bwdcnNWy6YPw355jXVYn+DlL6Lc"
		keyContentWithoutName, err := NewSecureAccessPublicKeyContent(rawValidSecureAccessPublicKeyContentWithoutName)
		if err != nil {
			t.Fatalf("SecureAccessPublicKeyContentParseFailed: %v", err)
		}

		_, err = keyContentWithoutName.ReadOnlyKeyName()
		if err == nil {
			t.Error("ReadOnlyKeyNameShouldFailForNamelessKey")
		}
	})

	t.Run("ReadFingerprint", func(t *testing.T) {
		rawValidSecureAccessPublicKeyContentWithName := "ssh-rsa  myMachine@pop-os"
		keyContentWithName, err := NewSecureAccessPublicKeyContent(rawValidSecureAccessPublicKeyContentWithName)
		if err != nil {
			t.Fatalf("SecureAccessPublicKeyContentParseFailed: %v", err)
		}

		_, err = keyContentWithName.ReadFingerprint()
		if err != nil {
			t.Errorf("ReadFingerprintFailed: %v", err)
		}
	})
}
