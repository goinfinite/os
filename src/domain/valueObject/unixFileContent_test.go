...
func TestUnixFileContent(t *testing.T) {
	t.Run("ValidUnixFileContent", func(t *testing.T) {
		validUnixFileContents := []string{
		...
		for _, extension := range validUnixFileContents {
			_, err := NewUnixFileContent(extension)
			if err != nil {
	...
	t.Run("InvalidUnixFileContent", func(t *testing.T) {
		invalidUnixFileContents := []string{
			"",
			"ab123123sadasdbbb",
			"asjklfjskldgnsdmfnmxncsahoidjwqiejqelk",
		}
		for _, extension := range invalidUnixFileContents {
			_, err := NewUnixFileContent(extension)
			if err == nil {
