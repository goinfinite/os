...
func TestNetworkPort(t *testing.T) {
	t.Run("ValidNetworkPort", func(t *testing.T) {
		validNetworkPorts := []interface{}{
			"8080", int(443), int8(80), int16(8000), int32(8080), int64(8443),
			uint(443), uint8(80), uint16(8000), uint32(8080), uint64(8443),
			float32(8080), float64(8443),
		}
		...
		for _, networkPort := range validNetworkPorts {
			...
			if err != nil {
				t.Errorf(
					"Expected no error for '%v', got '%s'", networkPort, err.Error(),
				)
			}
		}
	})
	...
	t.Run("InvalidNetworkPort", func(t *testing.T) {
		...
		for _, networkPort := range invalidNetworkPorts {
			_, err := NewNetworkPort(networkPort)
			if err == nil {
				t.Errorf("Expected error for '%v', got nil", networkPort)
			}
		}
	})
}
