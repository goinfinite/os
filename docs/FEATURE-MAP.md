...
3. `src/presentation/ui/presenter/runtimes/` — Web dashboard runtime configuration UI; distinguishes an unmapped virtual host from a failed config read and reports the latter
...
9. `src/infra/runtime/` — Runtime environment setup and version management; reads PHP module compatibility from the installed service assets, applies all module changes, restarts once, and verifies final module states; every runtime conf write — settings, version, hostname, and virtual host creation — goes through the tk file clerk, which replaces matches atomically; every restart validates the web server config first
